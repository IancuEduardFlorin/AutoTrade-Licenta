                                        ...
                                        <div style={styles.cardBody}>
                                            ...
                                            <div style={styles.cardFooter}>
                                                ...
                                                <button
                                                    style={{
                                                        ...styles.compareBtn,
                                                        background: compareList.find(c => c.id === anunt.id) ? 'var(--accent-tint-strong)' : 'transparent',
                                                        borderColor: compareList.find(c => c.id === anunt.id) ? 'var(--border-accent)' : 'var(--border)',
                                                        color: compareList.find(c => c.id === anunt.id) ? 'var(--accent-light)' : 'var(--text-muted)',
                                                    }}
                                                    onClick={(e) => handleCompare(e, anunt)}
                                                    className="tag-hover"
                                                >
                                                    {compareList.find(c => c.id === anunt.id) ? '✓ Compare' : '⊕ Compare'}
                                                </button>
                                            </div>
                                        </div>
                                    </Link>
            ...
            {compareList.length > 0 && (
                <div style={styles.compareBar}>
                    <div style={styles.compareBarInner}>
                        <span style={styles.compareCount}>{compareList.length}/3 cars selected</span>
                        <div style={styles.compareNames}>
                            {compareList.map(c => (
                                <span key={c.id} style={styles.compareTag}>
                                    {c.marca} {c.model} {c.an}
                                    <button style={styles.compareTagRemove} onClick={(e) => { e.stopPropagation(); removeFromCompare(c.id); }}>✕</button>
                                </span>
                            ))}
                        </div>
                        <button style={styles.compareGo} className="btn-primary-glow" onClick={() => navigate('/compare')}>
                            Compare →
                        </button>
                    </div>
                </div>
...
const styles = {
    ...
    compareBar: {
        ...
        background: 'var(--glass-deep)', backdropFilter: 'var(--glass-blur-heavy)',
        WebkitBackdropFilter: 'var(--glass-blur-heavy)',
        borderTop: '1px solid var(--border-accent)',
        boxShadow: '0 -4px 24px rgba(0,0,0,0.4)',
    },
    compareBarInner: {
        maxWidth: '900px', margin: '0 auto', padding: '10px 20px',
        display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap',
    },
    compareCount: { fontSize: '13px', fontWeight: '500', color: 'var(--accent-light)', flexShrink: 0 },
    compareNames: { display: 'flex', gap: '8px', flex: 1, flexWrap: 'wrap' },
    compareTag: {
    ...
    compareGo: {
        background: 'var(--btn-gradient)', color: 'var(--text-primary)',
