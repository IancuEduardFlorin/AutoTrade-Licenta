            ...
            <div style={styles.categorii}>
                {categorii.map(cat => (
                    <button
                        ...
                        style={{
                            ...styles.catBtn,
                            background: filtru === cat ? 'var(--accent)' : 'var(--pill-bg)',
                            color: filtru === cat ? 'var(--text-primary)' : 'var(--text-muted)',
                            borderColor: filtru === cat ? 'var(--accent)' : 'var(--border)',
                        }}
            ...
            {articoleFiltrate.length === 0 ? (
                ...
                <div style={styles.grid}>
                    {articoleFiltrate.map(articol => (
                        <Link to={`/news/${articol.id}`} key={articol.id} style={styles.card}>
                            ...
                            <div style={styles.cardBody}>
                                ...
                                <div style={styles.cardExcerpt}>
                                    {articol.continut.substring(0, 120)}...
                                </div>
...
const styles = {
    ...
    catBtn: { borderWidth: '1px', borderStyle: 'solid', borderRadius: '20px', padding: '5px 14px', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit' },
    empty: { textAlign: 'center', color: 'var(--text-muted)', padding: '40px' },
    grid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' },
    card: { background: 'var(--bg-card)', borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border)', borderRadius: '12px', overflow: 'hidden', textDecoration: 'none', display: 'flex', flexDirection: 'column' },
    cardImg: { position: 'relative', height: '180px', background: 'var(--pill-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
    img: { width: '100%', height: '100%', objectFit: 'cover' },
    imgPlaceholder: { fontSize: '40px' },
    catBadge: { position: 'absolute', top: '10px', left: '10px', background: 'var(--accent)', color: 'var(--text-primary)', fontSize: '10px', padding: '3px 8px', borderRadius: '4px', fontWeight: '500' },
